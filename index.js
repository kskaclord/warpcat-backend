...
<script type="module">
  ...
  async function init() {
    try { await sdk.actions.ready(); okDot.style.background = '#0bd30b'; setStatus('Hazır!'); }
    catch(e){ setStatus('Hazır!'); }
    ...
    mintBtn.onclick = async () => {
      ...
      try {
        ...
        if (!r.ok) throw new Error('Tx hatası: ' + r.status);
        ...
        if (!acc.isConnected) throw new Error('Cüzdan yok');

        setStatus('Cüzdan açılıyor…');
        const hash = await sendTransaction(config, {
          chainId: Number(String(tx.chainId).split(':').pop() || ${CHAIN_ID_NUM}),
          to: tx.params.to,
          data: tx.params.data,
          value: BigInt(tx.params.value)
        });

        setStatus('Mint gönderildi…');
        const link = 'https://basescan.org/tx/' + hash;
        resultEl.innerHTML = 'Tx: <a class="link" href="' + link + '" target="_blank" rel="noopener">BaseScan’da gör</a>';
      } catch (err) {
        console.error(err);
        if (String(err).toLowerCase().includes('wallet')) {
          setStatus('Preview’da cüzdan yok, Frame açılıyor…');
...
async function handleTx(req, res) {
  ...
  if (!CONTRACT_ADDR) return res.status(500).json({ error: 'CONTRACT_ADDRESS eksik' });
  ...
  try {
    ...
    if (NEYNAR_WEBHOOK_SECRET) {
      ...
      if (signature !== expected) {
        console.warn('[WEBHOOK] ❌ imza hatalı');
...
app.listen(PORT, () => {
  console.log(`WarpCat çalışıyor → ${PUBLIC_BASE_URL}`);
